...
export function TaskEdit ({isOpen, onClose, dataEdit} : TaskEditModalProps){
    ...
    const mutation = useMutation({
        ...
        onSuccess: () => {
            ...
            alert.success('berhasil update task', {title: "Berhasil"})
            onClose(); 
            resetForm(); 
        },
        onError: (err: any) => {
            alert.error(`Gagal update: ${err.message}`, { title: "Error" });
        },
        onSettled: () => {
            setLoading(false);
        }
    ...
    async function handleUpdate(e: React.FormEvent) {
        ...
        console.log(payload);
        console.log("onok gaseh", dataEdit.id);
        mutation.mutate({ id: dataEdit.id, formData: payload });
    }

    useEffect(() => {
        if(dataEdit && isOpen){
            setJudul(dataEdit.judul);
            setPrioritas(dataEdit.prioritas);
            setTipeTugas(dataEdit.tipe_tugas);
            setTenggatWaktu(new Date(dataEdit.tenggat_waktu));
            setSelectedMesin(dataEdit.mesin);
            setTaskTeknisi(dataEdit.ditugaskan_ke);
        }
    }, [dataEdit, isOpen])
    useEffect(() => {
        resetForm();
    ...
    useEffect(() => {
        console.log("Meisn berubah:", selectedMesin);
    }, [selectedMesin])
    return(
                ...
                <div className="flex min-h-full justify-center p-4 items-center sm:p-0">
                    ...
                    <DialogPanel className="flex  p-6 transform  rounded-lg bg-white dark:bg-slate-800 text-left shadow-xl outline outline-blue-50 dark:outline-blue-950 transition-all h-fit mt-10 mb-10">
                        ...
                        <div className="flex flex-col px-1 w-[460px] shrink-0 ">
                            ...
                            <form onSubmit={handleUpdate} className="py-2 space-y-5 min-h-55 text-base dark:text-gray-400">
                                { dataEdit && dataEdit.dibuat_oleh === 'admin' && (
                                    <>
                                ...
                                <div className="mb-3">
                                    <Listbox value={taskTeknisi} onChange={setTaskTeknisi} multiple by="user_id">
